...
function CleanerProjectPage() {
  ...
  const loadProjectData = async () => {
    try {
      ...
      // Load project with customer and carpenter
      ...
      setCarpenter(projectData.carpenter);
  ...
  const handleStartFinishing = async () => {
    ...
    try {
      ...
      await supabase.from('project_updates').insert([{
        project_id: id,
        uploaded_by: user.id,
        message: 'Finishing started',
      }]);

      toast.success('Finishing started');
      loadProjectData();
    } catch (error) {
      console.error('Error starting finishing:', error);
      toast.error('Failed to start finishing');
    } finally {
  ...
  const handleCompleteFinishing = async () => {
    ...
    try {
      ...
      await supabase.from('project_updates').insert([{
        project_id: id,
        uploaded_by: user.id,
        message: 'Finishing completed, ready for delivery',
      }]);

      // Notify assistant (you could get the assistant's email from users table)
      // For now, we'll use a general notification
      await notificationService.notifyReadyForDelivery({
        assistantId: user.id, // In a real app, you'd get the actual assistant ID
      ...
      if (customer?.email) {
        await notificationService.notifyProjectComplete({
          customerEmail: customer.email,
          projectTitle: project.title,
        });
      }

      toast.success('Project marked as ready for delivery');
      loadProjectData();
    } catch (error) {
      console.error('Error completing finishing:', error);
      toast.error('Failed to complete finishing');
    } finally {
      setUpdatingStatus(false);
    }
  };
  ...
  if (loading) {
    return (
      <DashboardLayout userRole="cleaner" userName="Loading...">
  ...
  if (!project) {
    return (
      <DashboardLayout userRole="cleaner" userName={user?.full_name || 'Cleaner'}>
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex items-center justify-between">
        ...
        {carpenter && (
          <div className="card">
            <h3 className="text-lg font-semibold text-stone-900 mb-4">Carpenter Information</h3>
            <div className="flex items-start space-x-3">
              <Hammer className="h-5 w-5 text-stone-400 mt-0.5" />
              <div>
                <p className="font-medium text-stone-900">{carpenter.full_name}</p>
                <p className="text-sm text-stone-500">Assigned Carpenter</p>
              </div>
            </div>
          </div>
        )}
        ...
        {/* Finishing Actions */}
        <div className="card">
          <h3 className="text-lg font-semibold text-stone-900 mb-4">Finishing Actions</h3>
          <div className="flex flex-wrap gap-4">
            {project.status === 'ready_for_finishing' && (
              <button
              ...
              >
                ...
                {updatingStatus ? 'Starting...' : 'Start Finishing'}
              </button>
            )}
            
            {project.status === 'finishing' && (
              <button
                onClick={handleCompleteFinishing}
                disabled={updatingStatus}
                className="btn-primary flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Check className="h-5 w-5 mr-2" />
                {updatingStatus ? 'Completing...' : 'Complete Finishing'}
              </button>
            )}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    rows="3"
                    placeholder="Describe your finishing progress (e.g., 'Polishing completed', 'Final inspection done')"
                  />
                </div>

                <div>
                  <label className="label">Upload Images or Videos</label>
                  <div className="border-2 border-dashed border-stone-300 rounded-lg p-6 text-center">
                    ...
                    >
                      ...
                      <p className="text-stone-600">Click to upload files</p>
                      <p className="text-sm text-stone-500">Images and videos up to 50MB</p>
                    </label>
                  </div>
...
export default CleanerProjectPage;
